...
5. 外层项目阶段记录
   `F:\workspace\projects\AgentWorkspaceLab\agent-workspace\docs\ai-project\项目阶段记录.md`。
6. 仓库内事实审查 `docs/当前仓库事实审查.md`。
7. 当前源码、测试、Git 和构建产物。

发生冲突时，以可重复验证的源码、测试和 Git 事实为准，并在当前原子边界内修正文档。
`README.md`、`ARCHITECTURE.md` 和部分 `.github/` 文件仍包含上游产品说明；当前自用效率
路线不把这些公开发布资料作为当前开发范围的事实源。
...
当前执行路线已切换为“自用效率优先”。E001～E012 的 BASE-01/P1-01/P1-02 已完成，下一原子为
E013“保留 UTF-8 BOM 和 Unicode 正文”。唯一原子计划为 E001～E144，其中：

- E001～E139 可以在用户不在线时连续执行。
- E140 是第一个人工与外部环境门；未获得明确决定前不得进入，也不得把真实 CLI 登录、
  图片体验或最终用户验收表述为已完成。
- 原 Windows 开源 v1 发布计划完整保留并暂停在 A027，后续恢复公开发布时必须基于新事实
  重新排序。
- 每个原子必须保持一个行为目标、一个变更原因和一个可独立回退的提交。
- 本原子完成验证、差异审查、提交和推送前，不得开始下一原子。
- 每完成 4 个原子以及每个阶段边界，更新外层项目工作台和阶段记录，并向用户同步：
...
`Exxx-S01`/`Exxx-S02` 或 `Exxx-F01`/`Exxx-F02`，不得扩大为大提交。
...
- `.github/`：社区入口和 CI/Release；当前自用效率路线不进入公开发布治理，相关内容暂不作为执行入口。

仓库内 `docs/规划/自用效率优先完整代码原子任务方案.md` 是当前代码开发可提交、可推送的
计划副本。外层 `agent-workspace/docs/ai-project/` 继续保存领航工作台、阶段记录和历史计划；
后续若两处计划冲突，先停止并同步事实，不能任选一份继续。
...
- 本文件：有效开发规则。
- 自用效率优先原子任务方案：E001～E144 的唯一执行顺序和完成标准。
- 当前仓库事实审查：A001 时点的证据基线。
- 项目工作台：当前停点和跨上下文恢复。
- 项目阶段记录：标准路线阶段与历史。

当前检查点：A001～A026 已完成；E001～E012 已完成并推送，最近边界提交为 E009
`4df06eb`、E010 `4d3cbf9`、E011 `e3ccf1f`、E012 `7e119b1`。
Grid 不再启动每工作区上下文资源，Focused 只持有一个活动上下文，Review 只保留专用
审查资源；编辑器已支持小修改、串行保存、关闭门禁和换行/BOM 保真；旧发布计划停在 A027，
当前应用测试基线为 1512，下一原子为 E013。
