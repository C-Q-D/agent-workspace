...
//! Markdown 预览和异步加载必须遵守的边界。它不提前实现编辑器，也不创建生产
//! 状态源；生产路径仍以 `WindowSession`、`DisplayState` 和活动上下文派生规则为准。

/// 右侧上下文面板可以显示的互斥内容。
///
/// 该枚举只描述所有权契约，不代表 UI 已经实现这些页面。E005 以后新增功能时，
/// 只能在当前聚焦会话内切换这些内容，不能把它们提升为顶层 `DisplaySurface`。
