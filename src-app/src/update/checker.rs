...
/// patching the binary.
const DEFAULT_FEED_URL: &str = crate::product_identity::LATEST_RELEASE_API_URL;
...
pub(crate) fn check_github_release(
    ...
    if let Ok(forced_version) = std::env::var("PANEFLOW_DEV_FORCE_UPDATE") {
        ...
        if !version.is_empty() && Version::parse(&version).is_ok() {
            ...
            return UpdateStatus::Available {
                version,
                url: crate::product_identity::RELEASES_URL.to_string(),
                asset_url: None,
                asset_format: None,
            };
        }
    ...
    let response = ureq::get(&feed_url)
        ...
        .build()
        .header("User-Agent", &format!("agent-workspace/{CURRENT_VERSION}"))
...
mod tests {
    ...
    fn make_asset(name: &str) -> GitHubAsset {
        GitHubAsset {
            ...
            browser_download_url: format!(
                "https://github.com/C-Q-D/agent-workspace/releases/download/v0/{name}"
            ),
        }
    }
    ...
    fn https_allowlisted_host_allowed_in_release() {
        ...
        assert!(is_allowed_update_url_impl(
            "https://github.com/C-Q-D/agent-workspace/releases/download/v1/x.tar.gz",
            false
        ));
    }
